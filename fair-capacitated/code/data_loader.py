...
class DataLoader(object):
	...
	def __init__(self, source, fair_column, fair_values, distance_columns):
		...
		assert len(distance_columns) > 1, "Needs at least one distance column."
	...
	def load(self, normalize=False):
		...
		if self.source == "student-por":
			self.data = pd.read_csv("data/student-por.csv", sep=";")
		elif self.source == "student-mat":
			self.data = pd.read_csv("data/student-mat-encode.csv", sep=",")
		elif self.source == "student-mat-encode":
			self.data = pd.read_csv("data/student-mat-encode.csv", sep=",")
		elif self.source == "student-por-encode":
			self.data = pd.read_csv("data/student-por-encode.csv", sep=",")
		elif self.source == 'oulad':
			self.data = pd.read_csv("data/oulad-encode.csv", sep=",")
		elif self.source == 'pisa-encode':
			self.data = pd.read_csv("data/pisa2009-clean.csv")
		elif self.source =='mooc':
			self.data = pd.read_csv("data/mooc-6.002x-encode.csv")
	...
	def split(self, split_size, random_state):
		...
		"""
		#df = self.data[self.data[self.fair_column]==1].sample(max(split_size), random_state=random_state,replace=True).append(
		#	self.data[self.data[self.fair_column]==0].sample(min(split_size), random_state=random_state,replace=True), ignore_index=True)
		#df = df.sample(frac=1, random_state=random_state,replace=True)
		df= self.data[self.data[self.fair_column]==1].sample(max(split_size), random_state=random_state,replace=True)
		#print(df)
		#df = self.data[self.data[self.fair_column]==1].sample(max(split_size), random_state=random_state,replace=True).append(
		#	self.data[self.data[self.fair_column]==0].sample(min(split_size), random_state=random_state,replace=True), ignore_index=True)
		#print(len(df))

		df1 = self.data[self.data[self.fair_column] == 0].sample(min(split_size), random_state=random_state, replace=True)
		#print(df1)
		#print(len(df1))
		...
		#df = df.sample(frac=1, random_state=random_state,replace=True)
		df_result = df_new.sample(frac=1, random_state=random_state, replace=False)
		#self.data = df.reset_index(drop=True)
		self.data = df_result.reset_index(drop=True)

		fair_df = self.data[self.data[self.fair_column]==1]
		#print(fair_df)
		#print(len(fair_df))
